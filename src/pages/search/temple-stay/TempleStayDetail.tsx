...
import { ArrowLeft, Home, Heart, Share, MapPin, Globe, Calendar, User } from 'lucide-react';
      ...
      <div className="w-full max-w-[480px] sm:max-w-[600px] md:max-w-[768px] lg:max-w-[1024px] mx-auto">
        ...
        <div className="px-5 py-4 bg-white">
          <div className="flex justify-between items-start mb-1.5">
            <h1 className="text-base font-bold text-[#222] font-pretendard">{templeStay.name}</h1>
          ...
          <div className="flex items-center gap-1 mb-4">
            ...
            <span className="text-xs text-[#111111] font-pretendard">
          ...
          <div className="flex items-center gap-1 mb-7">
            ...
            <span className="text-xs text-[#111111] font-pretendard">www.bodhis.com</span>
          </div>
          ...
          <div className="w-full border border-[rgba(153,153,153,0.2)] rounded-xl p-3 mb-6 shadow-sm">
            <div className="flex justify-between gap-2.5">
              {scheduleTimes.map((schedule, index) => (
                ...
                >
                  <span className="text-white text-[8px] font-medium font-pretendard">{schedule.time}</span>
                </div>
              ))}
            </div>
          ...
          {/* Price Information */}
          <h2 className="text-base font-bold text-[#222] font-pretendard mb-4">이용요금</h2>
          <div className="mb-6">
            <p className="text-lg font-bold text-[#DE7834] font-pretendard">{formatPrice(templeStay.price)}</p>
            <p className="text-xs text-gray-500 font-pretendard">총 {templeStay.duration} (세금 포함)</p>
          </div>
          
          {/* Notes */}
          <h2 className="text-base font-bold text-[#222] font-pretendard mb-4">유의사항</h2>
          <div className="mb-6">
            <p className="text-sm text-[#555] font-pretendard leading-relaxed">
              {templeStay.description}
            </p>
          </div>
          
          {/* Program Schedule */}
          <h2 className="text-base font-bold text-[#222] font-pretendard mb-4">프로그램 일정</h2>
          <div className="space-y-3 mb-6">
            {scheduleTimes.map((schedule, index) => (
              <div key={index} className="flex">
                <div className="w-24 text-sm font-medium font-pretendard">{schedule.time}</div>
                <div>
                  <p className="text-sm font-medium font-pretendard">{schedule.activity}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          {/* Book Button */}
          <button className="w-full bg-[#DE7834] text-white py-3 rounded-md text-base font-bold font-pretendard">
            예약하기
          </button>
        </div>
      </div>
